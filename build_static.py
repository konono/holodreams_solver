...
def build():
    ...
    with open(ROOT / "index.html", encoding="utf-8") as f:
        template = f.read()
    shared_render_js = (ROOT / "shared_render.js").read_text(encoding="utf-8")
...
<div class="container">
  ...
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;font-size:0.85rem;color:#8899aa;flex-wrap:wrap">
    ...
    <span style="color:#3a4f66">|</span>
    <label>曲:</label>
    <span id="songSelectWrapper" style="position:relative;display:inline-block;vertical-align:middle">
      <input id="songSearchInput" type="text" placeholder="曲名で検索…" autocomplete="off"
        style="background:#1e2d3d;border:1px solid #3a4f66;color:#c8d8e8;padding:4px 6px;border-radius:4px;font-size:0.75rem;width:220px;box-sizing:border-box">
      <div id="songDropdown" style="display:none;position:absolute;top:100%;left:0;width:320px;max-height:300px;overflow-y:auto;background:#1a2a3a;border:1px solid #3a4f66;border-radius:4px;z-index:1000;margin-top:2px;font-size:0.75rem">
      </div>
      <input type="hidden" id="songSelect" value="">
    </span>
...
SHARED_RENDER_PLACEHOLDER
<script>
const CARDS = {cards_json};
const LEVEL_TABLES = {level_tables_json};
const levelTables = LEVEL_TABLES;
...
function initSongSearch() {{
  ...
  let highlightIdx = -1;
  const songsSorted = SONGS_LIST.slice().sort((a, b) => a.name.localeCompare(b.name, 'ja'));

  function renderDropdown(filter) {{
    dropdown.innerHTML = "";
    const defaultItem = document.createElement("div");
    defaultItem.textContent = "汎用（192秒）";
    defaultItem.style.cssText = "padding:4px 8px;cursor:pointer;color:#8899aa";
    defaultItem.addEventListener("mousedown", e => {{ e.preventDefault(); selectSong(""); }});
    ...
    for (const s of songsSorted) {{
      ...
      item.style.cssText = "padding:4px 8px;cursor:pointer;color:#c8d8e8;display:flex;justify-content:space-between";
      item.innerHTML = `<span>${{escapeHtml(s.name)}}</span><span style="color:#6a7a8a;margin-left:12px;white-space:nowrap">${{s.playing_seconds}}秒</span>`;
      item.addEventListener("mousedown", e => {{ e.preventDefault(); selectSong(s.id); }});
    ...
    if (count === 0 && q) {{
      ...
      empty.style.cssText = "padding:4px 8px;color:#556677";
  ...
  input.addEventListener("focus", () => {{ input.value = ""; renderDropdown(""); dropdown.style.display = ""; }});
  input.addEventListener("input", () => {{ renderDropdown(input.value); dropdown.style.display = ""; }});
  input.addEventListener("blur", () => {{ setTimeout(() => {{ dropdown.style.display = "none"; restoreInputLabel(); }}, 150); }});
  input.addEventListener("keydown", e => {{
    const items = dropdown.querySelectorAll("div[data-id], div:first-child");
    ...
    else if (e.key === "Enter") {{ e.preventDefault(); if (highlightIdx >= 0 && items[highlightIdx]) items[highlightIdx].dispatchEvent(new Event("mousedown")); }}
    else if (e.key === "Escape") {{ input.blur(); }}
  }});

  function updateHighlight(items) {{
    items.forEach((el, i) => {{ el.style.background = i === highlightIdx ? "#2a3a4a" : ""; }});
    if (items[highlightIdx]) items[highlightIdx].scrollIntoView({{ block: "nearest" }});
  }}
}}

function restoreInputLabel() {{
  const id = document.getElementById("songSelect").value;
  const input = document.getElementById("songSearchInput");
  if (id) {{
    const s = SONGS_LIST.find(s => s.id === id);
    if (s) input.value = `${{s.name}} (${{s.playing_seconds}}秒)`;
  }} else {{
    input.value = "";
  }}
}}
...
function selectSong(id) {{
  ...
  document.getElementById("diffSelect").style.display = id ? "" : "none";
  restoreInputLabel();
}}
...
function getSelectedSongLength() {{
  ...
  if (!id) return null;
  const s = SONGS_LIST.find(s => s.id === id);
  return s ? s.playing_seconds : null;
}}
  ...
  w.onmessage = function(ev) {{
    ...
    if (ev.data.type === "progress") {{
      ...
      try {{
        document.getElementById("progressFill").style.width = "100%";
        document.getElementById("progressText").textContent =
          `完了！ ${{(ev.data.total_combinations || ev.data.candidate_pool || 0).toLocaleString()}} 通りを評価`;
        expandResults();
...
function restoreFromHistory(idx) {{
  ...
  selectSong(entry.settings?.songId || "");
