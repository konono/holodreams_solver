...
│  ┌── 総合力に効く ──────────────────┐  ┌── スコアボーナスに効く ───────┐ │
│  │                                  │  │                              │ │
│  │  ① パラメータUP（衣装）         │  │  ④ スコアUP（アクティブ）    │ │
│  │  ② パラメータUP（パッシブ）     │  │  ⑤ スコアサポート（衣装）    │ │
│  │                                  │  │  ⑥ スコアサポート（パッシブ）│ │
│  │                                  │  │  ⑦ スコアサポート（SP）     │ │
│  │                                  │  │  ⑧ 発動率UP（SP）          │ │
│  └──────────────────────────────────┘  └──────────────────────────────┘ │
│                                                                          │
│  ※ ⑧ は直接スコアに効かず、④の発動確率を上げることで間接的に効く      │
└──────────────────────────────────────────────────────────────────────────┘
```
...
│  │ ユニットスコア    │      │ ライブスコア              │    │
│  │                  │      │                          │    │
│  │ ゲーム内表示の   │      │ 実際にライブをプレイして  │    │
│  │ チーム評価値     │      │ 得られるスコア            │    │
│  │                  │      │                          │    │
│  │ 曲に依存しない   │      │ 曲・判定・RNGに依存する  │    │
│  │ 決定論的         │      │ 確率的                    │    │
│  └──────────────────┘      └──────────────────────────┘    │
│         ↑                            ↑                      │
│    v2で再現済み               v3 Timeline Engineで推定       │
└─────────────────────────────────────────────────────────────┘
```
...
- `combo_100`: 100コンボ以上で発動 → 曲序盤のみ不成立
...
┌──────────────────── 10秒間のスコア寄与 ────────────────────┐
│                                                            │
│  状態              │  Active  │ Support │  倍率  │  寄与  │
│  ──────────────────┼──────────┼─────────┼────────┼────────│
│  両方発動          │   120%   │  160%   │  5.72  │  ★★★  │
│  Activeのみ        │   120%   │    0%   │  2.20  │  ★★   │
│  Supportのみ       │     0%   │  160%   │  2.60  │  ★★   │
│  どちらもなし      │     0%   │    0%   │  1.00  │  ★    │
│                                                            │
│  ← 160% Supportの価値はActive発動中に最大化される         │
└────────────────────────────────────────────────────────────┘
```
...
```
               ...
               数百万通り
                    ...
                    ▼
         ┌──────────────────┐
         │ Legacy Fast Solver │ ← ユニットスコアで高速全探索
         │ E[max] 時間平均    │    全組み合わせ × 24通り(4!)
         └────────┬─────────┘
                  │
             Top 200 候補
                  │
                  ▼
       ┌──────────────────────┐
       │ Timeline Reranker     │ ← 曲別ライブ期待スコアで精密評価
       │ 5! = 120 順列        │    200候補 × 120 = 24,000 評価
       │ note密度 × combo     │
       │ SP配置 × Active重複  │
       └──────────┬───────────┘
                  │
